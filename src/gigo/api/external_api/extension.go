...
func (s *HTTPServer) ExtensionFiles(w http.ResponseWriter, r *http.Request) {
	_, parentSpan := otel.Tracer("gigo-core").Start(r.Context(), "extension-files-http")
	...
	if fileMime == "" {
		...
		if mimeReader == nil {
			// handle error internally
			s.handleError(w, fmt.Sprintf("file not found: %s", path), r.URL.Path, "ExtensionFiles", r.Method, r.Context().Value(CtxKeyRequestID),
		...
		if err != nil {
			// handle error internally
			s.handleError(w, "failed to read mime buffer", r.URL.Path, "ExtensionFiles", r.Method, r.Context().Value(CtxKeyRequestID),
				network.GetRequestIP(r), userName, userId, http.StatusInternalServerError, "internal server error", err)
			// exit
			return
		}
		...
		if err != nil {
			// handle error internally
			s.handleError(w, "failed to determine mime type", r.URL.Path, "ExtensionFiles", r.Method, r.Context().Value(CtxKeyRequestID),
				network.GetRequestIP(r), userName, userId, http.StatusInternalServerError, "internal server error", err)
			// exit
			return
		}
	...
	if err != nil {
		// handle error internally
		s.handleError(w, "failed to copy file", r.URL.Path, "ExtensionFiles", r.Method, r.Context().Value(CtxKeyRequestID),
			network.GetRequestIP(r), userName, userId, http.StatusInternalServerError, "internal server error", err)
		// exit
		return
	}
	...
	s.logger.LogDebugExternalAPI("function execution successful", r.URL.Path, "ExtensionFiles", r.Method,
