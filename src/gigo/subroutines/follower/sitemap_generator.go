...
func GenerateSiteMap(ctx context.Context, db *ti.Database, js *mq.JetstreamClient, storageEngine storage.Storage, nodeId int64, host string, logger logging.Logger) {
	...
	if err != nil {
		...
		if err != nil {
			logger.Errorf("(sitemap_gen: %d) failed to create session key consumer: %v", nodeId, err)
			return
		}
	}
	sub, err := js.PullSubscribe(streams.SubjectMiscSessionCleanKeys, "gigo-core-follower-session-keys", nats.AckExplicit())
	if err != nil {
		logger.Errorf("(sitemap_gen: %d) failed to create session key subscription: %v", nodeId, err)
		return
	}
	...
	for res.Next() {
		...
		if err != nil {
			logger.Errorf("(sitemap_gen: %d) unable to parse post struct: %w", nodeId, err)
			return
		}
		...
		err = sm.Add(&loc)
		if err != nil {
			logger.Errorf("(sitemap_gen: %d) unable to add post to the sitemap: %w", nodeId, err)
			return
		}
	}
	...
	if err != nil {
		logger.Errorf("(sitemap_gen: %d) unable to get posts from database: %w", nodeId, err)
		return
	}
	defer res.Close()
	...
	if err != nil {
		logger.Errorf("(sitemap_gen: %d) unable to write sitemap to buffer: %w", nodeId, err)
		return
	}
	...
	if err != nil {
		logger.Errorf("(sitemap_gen: %d) unable to save sitemap to storage engine: %w", nodeId, err)
		return
	}
